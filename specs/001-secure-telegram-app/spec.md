...
- Q: When `rg.dev-secure-service.enabled` is absent, should the application use the identity-service implementation or fail startup? → A: Missing property selects the identity-service implementation; startup fails only if zero or multiple facade implementations are active.
...
- **FR-020**: Application configuration MUST select exactly one facade implementation: `rg.dev-secure-service.enabled=true` selects the development implementation, while `false` or an absent property selects the identity-service implementation. Runtime wiring that provides zero or multiple facade implementations MUST fail application startup.
